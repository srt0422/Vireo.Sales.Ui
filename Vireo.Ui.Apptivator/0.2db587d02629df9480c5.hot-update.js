...
/* harmony reexport (binding) */ __webpack_require__.d(__webpack_exports__, "b", function() { return __WEBPACK_IMPORTED_MODULE_0__customersCollection__["a"]; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__chargesCollection__ = __webpack_require__(2);
/* unused harmony reexport chargesCollection */
...
server.register([{
    ...
    options: {
        "headers": ["Accept", "Authorization", "Content-Type", "content-type", "If-None-Match", "Accept-language"],
...
server.route({
    ...
    handler: {
        async await(request, reply) {
            ...
            try {
                __WEBPACK_IMPORTED_MODULE_2__data_collections__["a" /* appContentsCollection */].insert(request.payload);
            }
            catch (e) {
...
server.route({
    ...
    handler: {
        async await(request, reply) {
            let customers = await __WEBPACK_IMPORTED_MODULE_2__data_collections__["b" /* customersCollection */].getAll();
